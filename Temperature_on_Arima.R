...
acf(monthly_avg_time)
pacf(monthly_avg_time)

MannKendall((diff((log(monthly_avg_time))))) #detrend
adf.test((diff(log(monthly_avg_time)))) # Stationary after trend removal
friedman.test()
acf((diff(log(monthly_avg_time))))
pacf(diff(log(monthly_avg_time)))
model2 = auto.arima((monthly_avg_time), ic = "aic", trace = T) # ARIMA(2,0,1)
